...
public class SessionCommandUnitTest
{
    ...
    {
        var cmd = new AddProfileToSessionCommand { ProfileId = new Guid("13d54b8e-2c1f-4e35-49fb-08db94cbee6d"), SessionId = new Guid("13d54b8e-2c1f-4e35-49fb-08db94cbee6d") };
        var handler = new AddProfileToSessionCommandHandler(_mockSessionService.Object, _mapper);

        var result = handler.Handle(cmd, new CancellationToken());

        Assert.NotNull(result);
        _mockSessionService.Verify(mock => mock.AddProfileToSession(_mapper.Map<AddPlayerProfileToSessionDTO>(cmd)));
    }
}
